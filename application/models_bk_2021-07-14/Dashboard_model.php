...
class Dashboard_model extends CI_Model {
    ...
    public function getAnalyticsToday(){
        ...
        //sumar el costo x unidades de cada lote al costo del producto por las unidades de linea de producto
        //idea: cambiar costo de tabla de productos a tabla inventario
        $query1 = $this->db->query("SELECT ROUND((SUM(lp.precio_producto) - SUM(lp.costo_unitario*lp.unidades)) - (SELECT IFNULL(SUM(monto),0) FROM gastos WHERE fecha = CURDATE()),2) as 'Utilidad' 
        ...
        $query2 = $this->db->query("SELECT ROUND((SUM(lp.precio_producto) - SUM(lp.costo_unitario*lp.unidades)) - (SELECT IFNULL(SUM(monto),0) FROM gastos WHERE fecha = CURDATE()- INTERVAL 1 DAY),2) as 'utilidadAyer' 
        FROM lineas_pedido lp 
        LEFT JOIN pedidos ped ON ped.id = lp.pedido_id 
