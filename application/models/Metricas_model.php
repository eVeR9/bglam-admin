 ...
 * /////////////////////////////////////////////////////////////////////////////////////
 * FECHA: 28/07/2021
 * SE NECESITA REALIZAR UNA MODIFICACION EN EL QUERY, YA QUE SI SE LE REALIA UN DESCUENTO A
 * UN PEDIDO NO SE VERA REFLEJADO EN LOS REPORTES DE UTILIDAD.
 * NO PODEMOS METER ESTE TIPO DE TRANSACCIONES COMO GASTOS, YA QUE REALMENTE NECESITAMOS 
 * MANDAR TREAER LA COLUMNA DE PRECIO DE LA TABLA PEDIDOS EN LA OPERACION PARA EXTRAER LA UTILIDAD
 * 
 * #prod
 ...
 *       
 * EN EL SIGUIENTE QUERY CAMBIAMOS LA COLUMNA DE PRECIO DE PRODUCTO DE LA TABLA LINEAS DE PEDIDO POR
 * PRECIO DE PRODUCTO DE PEDIDOS
 * #new improved feature
 *       SELECT ROUND((SUM(ped.total_pedido) - SUM(lp.costo_unitario*lp.unidades)) - 
 *       (SELECT IFNULL(SUM(monto),0) FROM gastos WHERE fecha = CURDATE()),2) as 'Utilidad' 
 *       FROM lineas_pedido lp 
 *       LEFT JOIN pedidos ped ON ped.id = lp.pedido_id 
 *       WHERE DATE_FORMAT(ped.created_at, '%Y-%m-%d') = DATE(NOW()) AND ped.activo_inactivo = '1';
...
class Metricas_model extends CI_Model
{
    ...
    public function getAnalyticsToday(){
        ...
        $query6 = $this->db->query("SELECT ROUND(IFNULL((SUM(total_pedido)/COUNT(id)),0),2) as ticket_promedio FROM pedidos 
        WHERE DATE_FORMAT(created_at, '%Y-%m-%d') = DATE(CURDATE()) AND activo_inactivo = '1'");
    ...
    public function getAnalyticsYesterday(){
        ...
        $this->db->query("SET @pedidos_antier = (SELECT IFNULL(SUM(total_pedido),0) FROM pedidos WHERE DATE_FORMAT(created_at, '%Y-%m-%d') = CURDATE() - INTERVAL 2 DAY  AND activo_inactivo = '1');");
        $query2 = $this->db->query(
        "SELECT ROUND(IFNULL((@pedidos_antier - SUM(lp.costo_unitario*lp.unidades)),0) - 
        ...
        $query3 = $this->db->query("SELECT ROUND(IFNULL(SUM(total_pedido),0),2) as total_ventas_ayer FROM pedidos WHERE DATE_FORMAT(created_at, '%Y-%m-%d') = DATE(CURDATE() - INTERVAL 1 DAY) AND activo_inactivo = '1'");
        $query4 = $this->db->query("SELECT ROUND(IFNULL(SUM(total_pedido),0),2) as total_ventas_antier FROM pedidos WHERE DATE_FORMAT(created_at, '%Y-%m-%d') = DATE(CURDATE() - INTERVAL 2 DAY) AND activo_inactivo = '1'");
        ...
        $query6 = $this->db->query("SELECT ROUND(IFNULL((SUM(total_pedido)/COUNT(id)),0),2) as ticket_promedio_ayer FROM pedidos 
        WHERE DATE_FORMAT(created_at, '%Y-%m-%d') = DATE(CURDATE() -1) AND activo_inactivo = '1'");
    ...
    public function getAnalyticsThisWeek(){
        ...
        //analizar este query
        $this->db->query("SET @pedidos_esta_semana = (SELECT IFNULL(SUM(total_pedido),0) FROM pedidos WHERE WEEK(created_at, 1) = WEEK(CURDATE()) AND activo_inactivo = '1');");
        $query1 = $this->db->query(
            "SELECT ROUND(IFNULL((@pedidos_esta_semana - SUM(lp.costo_unitario*lp.unidades)),0) - 
        ...
        $this->db->query("SET @pedidos_semana_pasada = (SELECT IFNULL(SUM(total_pedido),0) FROM pedidos WHERE WEEK(created_at, 1) = WEEK(CURDATE())-1 AND activo_inactivo = '1');");
        $query2 = $this->db->query(
            "SELECT ROUND(IFNULL((@pedidos_semana_pasada - SUM(lp.costo_unitario*lp.unidades)),0) - 
        ...
        $query3 = $this->db->query("SELECT ROUND(IFNULL(SUM(total_pedido),0),2) as total_ventas_esta_semana FROM pedidos 
        WHERE WEEK(created_at, 1) = WEEK(CURDATE()) AND activo_inactivo = '1'");

        $query4 = $this->db->query("SELECT ROUND(IFNULL(SUM(total_pedido),0),2) as total_ventas_semana_pasada FROM pedidos 
        ...
        $query6 = $this->db->query("SELECT ROUND(IFNULL((SUM(total_pedido)/COUNT(id)),0),2) as ticket_promedio_esta_semana FROM pedidos 
        WHERE WEEK(created_at, 1) = WEEK(CURDATE()) AND activo_inactivo = '1'");
    ...
    public function getAnalyticsLastWeek(){
        ...
        //ultimos15Dias
        $this->db->query("SET @pedidos_ultimos_15dias = (SELECT IFNULL(SUM(total_pedido),0) FROM pedidos WHERE created_at BETWEEN CURDATE() - INTERVAL 15 DAY AND CURDATE() AND activo_inactivo = '1');");
        $query1 = $this->db->query(        
        "SELECT ROUND(IFNULL((@pedidos_ultimos_15dias - SUM(lp.costo_unitario*lp.unidades)),0) - 
        ...
        //For variables of query 2 and query 4
        $this->db->query(
            "SET @pedidos_ultimos_30Dias = (SELECT IFNULL(SUM(total_pedido),0)
            FROM pedidos 
        ...
        $this->db->query(
        "SET @utilidad_ultimos_30dias = (SELECT ROUND(IFNULL((@pedidos_ultimos_30Dias - SUM(lp.costo_unitario*lp.unidades)),0) - 
        ...
        $this->db->query(
            "SET @utilidad_ultimos_15dias = (SELECT ROUND( IFNULL((@pedidos_ultimos_15Dias - SUM(lp.costo_unitario*lp.unidades)),0 ) - 
        ...
        $query6 = $this->db->query("SELECT ROUND(IFNULL((SUM(total_pedido)/COUNT(id)),0),2) as ticket_promedio_semana_pasada FROM pedidos WHERE WEEK(created_at, 1) = WEEK(CURDATE()) -1 AND activo_inactivo = '1'");
    ...
    public function getAnalyticsOfMonth(){
        ...
        $this->db->query("SET @pedidos_este_mes = (SELECT ROUND(IFNULL(SUM(total_pedido),0),2) FROM pedidos WHERE MONTH(created_at) = MONTH(CURDATE()) AND activo_inactivo = '1')");
        $query1 = $this->db->query(
        "SELECT ROUND(IFNULL((@pedidos_este_mes - SUM(lp.costo_unitario*lp.unidades)),0) - 
        ...
        */
        ...
        $this->db->query("SET @pedidos_mes_anterior = (SELECT ROUND(IFNULL(SUM(total_pedido),0),2) FROM pedidos WHERE MONTH(created_at) = MONTH(CURDATE()) -1 AND activo_inactivo = '1')");
        $query5 = $this->db->query(
        "SELECT ROUND(IFNULL((@pedidos_mes_anterior - SUM(lp.costo_unitario*lp.unidades)),0) - 
        ...
        $query4 = $this->db->query("SELECT ROUND(IFNULL((SUM(total_pedido)/COUNT(id)),0),2) as ticket_promedio_mes_actual FROM pedidos WHERE MONTH(created_at) = MONTH(CURDATE()) AND activo_inactivo = '1'");
